...
class Producto{
    constructor(id,title,category,price,img,desc, fecha){
      this.id = id;
      this.title = title;
  ...
  for (let i = 0; i < lista_productos.length; i++){
    menu.add_producto(new Producto(lista_productos[i][0],lista_productos[i][1],lista_productos[i][2],lista_productos[i][3],lista_productos[i][4],lista_productos[i][5],lista_productos[i][6]));
  }
    ...
    for (let i = 0; i < menu.productos.length; i++){
      html_menu = html_menu + `<article class="menu-item">
        ...
        <div class="item-info">
          ...
          <p>
            Fecha de entrega: ${menu.productos[i].fecha}
          </p>
        </div>
        <button class="btn btn-secondary" type="button" onclick="pedir(${i})" >Pedir</button>
  ...
  function display_buttons(){
    ...
    for (let i = 0; i < categorias.length; i++){
      ...
      <button type="button" class="filter-btn btn btn-secondary" data-id=${categorias[i]}>
        ${categorias[i]}
      </button>
        `; 
    }
...
function pedir(index){
  ...
  mensaje.innerHTML = `<img class="photo" src="${menu.productos[index].img}"> 
  <h2>${menu.productos[index].title}</h2>
  <p> Precio: ${menu.productos[index].price}</p>
  <p> ${menu.productos[index].desc}</p>
  <p> Fecha de entrega: ${menu.productos[index].fecha}</p>
  `
  modal.click();
...
function pagar(){
  var modal = document.querySelector('#modal__button');
  //document.getElementById("modal__button__1").style.display = "none";
  var mensaje = document.querySelector('.modal-body');
  mensaje.innerHTML = `Compra realizada con exito`
